...
# Add Emscripten port include paths for SDL3 and OpenAL
if(DEFINED ENV{EMSDK})
    ...
    # Find OpenAL headers - they should be in include/AL/al.h
    set(EMSCRIPTEN_OPENAL_INCLUDE_DIR "")
    foreach(SYSROOT ${POSSIBLE_SYSROOTS})
        if(EXISTS "${SYSROOT}/include/AL/al.h")
            set(EMSCRIPTEN_OPENAL_INCLUDE_DIR "${SYSROOT}/include")
            message(STATUS "Found OpenAL headers at: ${EMSCRIPTEN_OPENAL_INCLUDE_DIR}")
            break()
        endif()
    endforeach()

    if(NOT EMSCRIPTEN_OPENAL_INCLUDE_DIR)
        message(WARNING "OpenAL headers not found in Emscripten SDK! Please ensure OpenAL port is installed: embuilder build openal")
    else()
        # Set as both cache and regular variable to ensure it's available
        set(EMSCRIPTEN_OPENAL_INCLUDE_DIR "${EMSCRIPTEN_OPENAL_INCLUDE_DIR}" CACHE PATH "OpenAL include directory for Emscripten" FORCE)
    endif()
endif()
...
set(EMSCRIPTEN_COMPILE_DEFINITIONS
    "OPENLOCO_WEB=1"
    "OPENLOCO_ARCHITECTURE=\"wasm\""
    "OPENLOCO_PLATFORM=\"Emscripten\""
)

# Note: Compile definitions are handled by the project's CMakeLists.txt via emscripten_private_compile_definitions
...
set(EMSCRIPTEN_LINK_FLAGS
    ...
    "-sUSE_SDL=3"                     # SDL3 audio via Web Audio API
...
set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${EMSCRIPTEN_CXX_FLAGS}" CACHE STRING "Emscripten C++ flags" FORCE)
set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} ${EMSCRIPTEN_EXE_LINKER_FLAGS}" CACHE STRING "Emscripten linker flags" FORCE)

# Define OPENLOCO_WEB for web-specific code
set(OPENLOCO_WEB TRUE CACHE BOOL "Building for Web/WebAssembly" FORCE)

message(STATUS "Configuring OpenLoco for Emscripten/WebAssembly")
message(STATUS "Build type: ${CMAKE_BUILD_TYPE}")
message(STATUS "Memory: 64MB initial, auto-growing")
message(STATUS "Audio: SDL3 audio (Web Audio API)")
